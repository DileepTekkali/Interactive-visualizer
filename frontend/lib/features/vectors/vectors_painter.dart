...
class VectorsPainter extends CustomPainter {
  ...
  VectorsPainter({
    required this.mag1, required this.angle1,
    required this.mag2, required this.angle2,
    required this.unit,
  ...
  void _drawVector(Canvas canvas, Size size, Offset start, double mag, double angleDeg, Color color, String label) {
    if (mag == 0) return;
    final rad = angleDeg * math.pi / 180;
    
    final endX = start.dx + mag * math.cos(rad);
    final endY = start.dy + mag * math.sin(rad);
    
    final p1 = toCanvas(start.dx, start.dy, size);
    // Interpolate end position based on animProgress
    final interpMag = mag * animProgress;
    final p2 = toCanvas(start.dx + interpMag * math.cos(rad), start.dy + interpMag * math.sin(rad), size);

    canvas.drawLine(p1, p2, Paint()..color = color..strokeWidth = 2.5);
    
    // Draw arrow head
    if (animProgress > 0.9) {
      final arrowAngle = math.atan2(p1.dy - p2.dy, p1.dx - p2.dx);
      final headL = 12.0;
      final arrowP1 = Offset(p2.dx + headL * math.cos(arrowAngle - math.pi / 6), p2.dy + headL * math.sin(arrowAngle - math.pi / 6));
      final arrowP2 = Offset(p2.dx + headL * math.cos(arrowAngle + math.pi / 6), p2.dy + headL * math.sin(arrowAngle + math.pi / 6));

      final path = Path()..moveTo(p2.dx, p2.dy)..lineTo(arrowP1.dx, arrowP1.dy)..lineTo(arrowP2.dx, arrowP2.dy)..close();
      canvas.drawPath(path, Paint()..color = color);

      // Label
      _drawText(canvas, '$label (${mag.toStringAsFixed(1)}$unit)', Offset(p2.dx + 5, p2.dy + 5), TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.bold));
    }
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (animProgress > 0.9 && (mag1 != 0 || mag2 != 0)) {
       ...
       final resAngle = math.atan2(resY, resX) * 180 / math.pi;

       // Draw dotted resultant
       final p1 = toCanvas(0, 0, size);
       final p2 = toCanvas(resX, resY, size);
       
       final dashPaint = Paint()..color = AppTheme.success..strokeWidth = 2..style = PaintingStyle.stroke;
       canvas.drawLine(p1, p2, dashPaint); // Simplified line for resultant
       
       _drawText(canvas, 'R (${resMag.toStringAsFixed(1)}$unit)', Offset(p2.dx - 20, p2.dy - 25), 
         TextStyle(color: AppTheme.success, fontSize: 12, fontWeight: FontWeight.bold));
    }
  }

  @override
  bool shouldRepaint(VectorsPainter old) =>
      old.mag1 != mag1 || old.angle1 != angle1 ||
      old.mag2 != mag2 || old.angle2 != angle2 ||
      old.animProgress != animProgress || old.unit != unit;
}
