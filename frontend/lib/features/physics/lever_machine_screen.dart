                    ...
                    children: [
                      ...
                      if (_data != null) ...[
                        if (_data?['success'] == true) ...[
                          _stat('Load Force (Weight)',
                              "${(_data?['load_force'] as num?)?.toDouble() ?? 0.0} N"),
                          _stat('Effort Required',
                              "${(_data?['effort_required'] as num?)?.toDouble() ?? 0.0} N"),
                          const SizedBox(height: 16),
                          Text(
                              'A mechanical advantage > 1 means less effort is needed to lift the load.',
...
class _LeverPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        Rect.fromLTRB(leftX - 20, center.dy - boxSize, leftX + 20, center.dy),
    ...
    if (er != null) {
      ...
      _drawArrow(canvas, Offset(rightX, center.dy - arrowLen - 10),
          Offset(rightX, center.dy), Colors.greenAccent);
    }
  }
