      ...
      child: Container(
        ...
        child: Column(
          children: [
            Padding(
                padding: const EdgeInsets.all(16),
                child: Text('Dipole Magnet Field Lines',
              ...
              child: AnimatedBuilder(
                ...
                builder: (_, __) => CustomPaint(
                  painter: _MagnetPainter(
                      strength: _strength, animValue: _animController.value),
                  size: Size.infinite,
                ),
              ),
            ),
                    ...
                    children: [
                      ...
                      if (_data != null) ...[
                        if (_data?['success'] == true) ...[
                          _stat('Grid Points',
                              "${(_data?['grid'] as List?)?.length ?? 0}"),
                          _stat('Max B-Field',
                              "${(_data?['max_b_field'] as num?)?.toDouble() ?? 0.0} T"),
                          const SizedBox(height: 16),
                          Text(
                              'Magnetic field loops from North (Red) to South (Blue). Field is strongest at the poles.',
                              style: GoogleFonts.inter(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    TextPainter(
        text: TextSpan(
            text: 'N',
            style:
                GoogleFonts.jetBrainsMono(color: Colors.white, fontSize: 20)),
        textDirection: TextDirection.ltr)
      ..layout()
      ..paint(canvas, Offset(center.dx - 40, center.dy - 10));
    TextPainter(
        text: TextSpan(
            text: 'S',
            style:
        ...
        textDirection: TextDirection.ltr)
      ..layout()
      ..paint(canvas, Offset(center.dx + 25, center.dy - 10));
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      canvas.drawOval(
          Rect.fromCenter(center: center, width: 140 + a, height: b * 2),
          paint);

      // Animating particles along the path (approximate)
      double t = (animValue + (i * 0.25)) % 1.0;
      if (t > 0.5) {
        // bottom half goes S -> N (wrong for outside magnet, actually N->S always outside)
      }
      ...
      double pxTop = center.dx -
          ((140 + a) / 2) * math.cos(partAngle); // Moves left to right
      double pyTop = center.dy - b * math.sin(partAngle);

      double pxBot = center.dx -
          ((140 + a) / 2) * math.cos(partAngle); // Moves left to right
