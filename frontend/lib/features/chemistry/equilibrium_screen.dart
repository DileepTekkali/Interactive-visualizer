...
class _EquilibriumScreenState extends State<EquilibriumScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _animController = AnimationController(vsync: this, duration: const Duration(seconds: 1), value: 0.5);
    _calculate();
  }
  ...
  Widget build(BuildContext context) {
    return MainLayout(title: 'Le Chatelier Principle', child: Column(children: [
      Expanded(child: Center(
        child: AnimatedBuilder(
          animation: _animController,
          builder: (_, __) => Transform.rotate(
            angle: (_animController.value - 0.5) * 0.5,
            child: Container(width: 300, height: 10, color: Colors.blueAccent),
          ),
        ),
      )),
      Padding(
        ...
        child: Container(
          ...
          child: Column(
            children: [
               Text('Exothermic Reaction: A + B ⇌ C + D + Heat', style: TextStyle(color: Colors.white, fontSize: 18)),
               const SizedBox(height: 16),
               ParameterSlider(label: 'Temperature Change (°C)', value: _tempChange, min: -100, max: 100, onChanged: (v) { setState(() => _tempChange = v); _calculate(); }),
               const SizedBox(height: 16),
               if (_result != null) ...[
                 if (_result?['success'] == true)
                    Text("Shift: \${_result?['shift'] ?? 'None'}", style: const TextStyle(color: Colors.orangeAccent, fontSize: 24, fontWeight: FontWeight.bold))
                 else
