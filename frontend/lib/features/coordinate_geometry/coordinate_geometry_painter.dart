...
class CoordinateGeometryPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = -10; i <= 10; i += 2) {
      ...
      if (i != 0) _drawText(canvas, '$i', Offset(origin.dx + 4, yPt.dy - 6), labelStyle);
    }
    ...
    // Line from A to B
    canvas.drawLine(screenP1, currentP2, Paint()..color = AppTheme.secondary.withOpacity(0.7)..strokeWidth = 2.5..strokeCap = StrokeCap.round);
    ...
    final dottedPaint = Paint()..color = AppTheme.axisLine.withOpacity(0.3)..strokeWidth = 1;
    ...
    // Labels
    _drawText(canvas, 'A(${p1.dx.toInt()}, ${p1.dy.toInt()})',
        screenP1 + const Offset(10, -20),
        TextStyle(color: AppTheme.accent, fontWeight: FontWeight.bold, fontSize: 12));
    _drawText(canvas, 'B(${p2.dx.toInt()}, ${p2.dy.toInt()})',
        currentP2 + const Offset(10, 10),
        TextStyle(color: AppTheme.secondary, fontWeight: FontWeight.bold, fontSize: 12));
    _drawText(canvas, 'M', mid + const Offset(8, -5),
        TextStyle(color: AppTheme.success, fontWeight: FontWeight.bold, fontSize: 11));
  }
